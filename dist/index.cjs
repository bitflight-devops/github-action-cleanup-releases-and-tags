...
// node_modules/@octokit/graphql/node_modules/node-fetch/lib/index.js
var require_lib3 = __commonJS({
  "node_modules/@octokit/graphql/node_modules/node-fetch/lib/index.js"(exports, module2) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    function _interopDefault(ex) {
      return ex && typeof ex === "object" && "default" in ex ? ex["default"] : ex;
...
var import_plugin_throttling = __toESM(require_dist_node13(), 1);
console.log("Loading MyOctokit");
var MyOctokit = import_utils.GitHub.plugin(import_plugin_throttling.throttling);
var oOptions = {
  throttle: {
...
function repoSplit(inputRepo) {
  ...
  if (import_github.context.payload.repository) {
    return {
      owner: import_github.context.payload.repository.owner.login,
      repo: import_github.context.payload.repository.name
    };
  }
  (0, import_core.setFailed)(`context.repo requires a GITHUB_REPOSITORY environment variable like 'owner/repo'`);
}
async function run() {
  try {
    console.log(`Event type is: ${import_github.context.eventName}`);
    const { number, ref } = import_github.context.payload;
    const branch = (0, import_core.getInput)("branch");
    const pr_number = (0, import_core.getInput)("pr_number");
    const repository = (0, import_core.getInput)("repository");
    const regex = (0, import_core.getInput)("regex");
    const br = branch || ref;
    const pr = pr_number || number;
    const searcher = pr || basename(br);
    if (!searcher) {
      (0, import_core.setFailed)("This is not a pull_request or delete event, and there was no pr_number, branch, or regex provided!");
    }
